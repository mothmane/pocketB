import { Component, OnInit, OnDestroy } from '@angular/core';
import { HttpResponse, HttpErrorResponse } from '@angular/common/http';
import { Subscription } from 'rxjs/Subscription';
import { JhiEventManager, JhiAlertService } from 'ng-jhipster';

import { BankReference } from './bank-reference.model';
import { BankReferenceService } from './bank-reference.service';
import { Principal } from '../../shared';

@Component({
    selector: 'jhi-bank-reference',
    templateUrl: './bank-reference.component.html'
})
export class BankReferenceComponent implements OnInit, OnDestroy {
bankReferences: BankReference[];
    currentAccount: any;
    eventSubscriber: Subscription;

    constructor(
        private bankReferenceService: BankReferenceService,
        private jhiAlertService: JhiAlertService,
        private eventManager: JhiEventManager,
        private principal: Principal
    ) {
    }

    loadAll() {
        this.bankReferenceService.query().subscribe(
            (res: HttpResponse<BankReference[]>) => {
                this.bankReferences = res.body;
            },
            (res: HttpErrorResponse) => this.onError(res.message)
        );
    }
    ngOnInit() {
        this.loadAll();
        this.principal.identity().then((account) => {
            this.currentAccount = account;
        });
        this.registerChangeInBankReferences();
    }

    ngOnDestroy() {
        this.eventManager.destroy(this.eventSubscriber);
    }

    trackId(index: number, item: BankReference) {
        return item.id;
    }
    registerChangeInBankReferences() {
        this.eventSubscriber = this.eventManager.subscribe('bankReferenceListModification', (response) => this.loadAll());
    }

    private onError(error) {
        this.jhiAlertService.error(error.message, null, null);
    }
}
